...
command below runs out of the box, as an integration smoke test. See
[Implementing FastSpeech2](#implementing-fastspeech2) below for the shape of
what is missing.
...
- **Durations need a source**: either an external forced aligner (e.g. the
  ...
  the model. Several published approaches exist for the latter; a literature
  search is worthwhile to avoid the external-tool step. Either way,
  ...
  > On the MFA route: MFA's pretrained acoustic models expect their own
  > phoneme dictionary, which generally will not match this template's
  > `--frontend phonemizer` (espeak-derived IPA, built dynamically per corpus,
  > see [Text frontends](#text-frontends)). Mixing the two means MFA's
  > alignment will not correspond to the model's token ids. Either keep the
  > default `g2p_en` (ARPAbet) frontend when using MFA (MFA's standard
  > English dictionaries are ARPAbet-based, so this is the compatible
  > pairing), or train or adapt an MFA acoustic model and dictionary on the
  > chosen phoneme inventory to use `phonemizer` with MFA together.
- **Pitch/energy targets**: this template's `scripts/preprocess.py` only
  extracts mel-spectrograms and text. Extracting per-frame or per-phoneme
  pitch and energy (and, on the external-aligner route, durations) is left to
  add, either in a copy of `scripts/preprocess.py` or a separate
  preprocessing pass, with `src/data/dataset.py`/`datamodule.py` extended to
  load and batch them alongside the text/mel pairs already there.
- **`forward()`'s `"loss"`** should be the combined objective (mel
  reconstruction, duration, pitch, and energy losses, with whatever weighting
  is chosen).
