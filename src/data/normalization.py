...
metadata.csv already ships pre-normalized transcripts (numbers and
abbreviations already expanded), so scripts/preprocess.py's default
`--normalize none` is correct and sufficient here. This hook exists purely as
an extension point for pointing the pipeline at a messier, non-LJSpeech
dataset later.

The "nemo" backend requires the `normalize` extra (`uv sync --extra normalize`)
and is Linux/WSL/conda-forge only, since its core dependency `pynini` has no
